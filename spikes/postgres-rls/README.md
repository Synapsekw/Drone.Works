...
- contract-preserving migrations leave the digest of customer-table owners, grants, policies, RLS, and `FORCE RLS` unchanged, while declared expansion migrations add exactly six remaining-resource tables, one export-request table, and two maintenance tables without changing existing table isolation;
