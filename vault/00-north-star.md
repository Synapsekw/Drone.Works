...
- **Phase:** Phase 0; P0-05 organization isolation is active, with relational RLS, deterministic organization-export generation, derived downloads and maintenance state, real-queue retry, reviewed migration isolation, and versioned core APIs proven.
- **Branch:** source commit `b3c9d32` is local on `main`, which is twenty-four commits ahead of `origin/main` before this vault-only closeout commit; nothing was pushed in this working block.
- **Completed evidence:** twenty-three customer tables run under forced RLS. Manager-authorized export requests freeze sanitized rows for nineteen operational collections; the worker deterministically emits a manifest, complete JSON data, flight CSV, and telemetry CSV, derives an organization-prefixed artifact, recovers through real pg-boss retry, finalizes once, and preserves uniform pilot/Beta download denial. Prior maintenance, signer, flight, administration, remaining-resource, migration, and queue evidence remains passing.
- **Verification baseline:** 29 native PostgreSQL integration tests and 78 existing host tests pass with zero skips/failures. JavaScript syntax, five pinned migration checksums, declared contract preservation/expansion, Git whitespace, deterministic file and bundle digests, role and cross-organization denials, contextless pooled reuse, strict queue payloads, loopback provider behavior, and real macOS parser network denial pass. Hosted run `29403024703` remains the latest Linux release-evidence baseline. No Docker, persistent PostgreSQL service, real object provider, or customer data was used.
- **Blocking evidence:** D-002 remains proposed because real provider-side URL/object/deletion behavior and permanent deletion paths are not yet executable; the logical export envelope also does not select a production ZIP/TAR container. Production credential delivery, externally retained database audit logs, and emergency operations remain P0-07 concerns. D-011 remains proposed pending atomic dispatch, worker termination, cancellation, and queue-age evidence; export effect idempotency under retry now passes. Broader fixture coverage remains permission-gated; production D-012 gates remain separate.
- **Next technical action:** continue P0-05 with the smallest permanent organization-deletion database proof: owner grace expiry, explicit worker organization context, customer-row cascade, a non-customer deletion receipt, idempotent retry, pooled reuse, and cross-organization denial. Keep real object/log/backup deletion outside the claim until provider evidence exists.
...
| P0-05 organization isolation | Active; relational, signer, queue, core API, deterministic export artifact, maintenance, and reviewed migration boundaries proven | Twenty-three-table forced RLS, composite ownership, pooled context, derived keys, bounded downloads, retry isolation, role-scoped `/api/v1/` behavior, idempotency, audit redaction, imported/user association origins, upload/import scope, immutable export data snapshots, deterministic JSON/CSV bundle bytes, derived export artifacts, strict queue references, derived maintenance usage, append-only completions, historical unlink, reversible deletion requests, single-owner enforcement, explicit migration elevation, independently owned migration audit, and declared isolation-contract preservation/expansion pass; real provider and permanent deletion remain open. |
