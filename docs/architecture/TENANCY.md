...
Permanent deletion uses a separate one-connection deletion pool and the same transaction-local setting, applied inside the reviewed security-definer function before any customer lookup or delete. The function locks and matches the current `pending_deletion` timestamp, enforces the 30-day grace boundary, deletes every child table in explicit dependency order, deletes the organization root, and appends the operational receipt atomically. `COMMIT` clears context before that backend is reused. Ordinary app, queue, and deletion-worker SQL cannot directly delete the root; only the narrow function is executable by the deletion worker.
...
- Extend isolation and deletion tests across cached organization-linked secrets, permanent flight deletion, external logs, backups, and provider deletion as those boundaries become executable.
