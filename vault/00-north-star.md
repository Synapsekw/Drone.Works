...
- **Phase:** Phase 0; P0-05 organization isolation is active, with relational RLS and derived object/download authorization proven through a signer boundary.
- **Branch:** source commit `f09ad43` is local on `main`, which is six commits ahead of `origin/main` before this vault-only closeout commit; nothing was pushed in this working block.
- **Completed evidence:** nine forced-RLS tables now include organization-owned raw-source and export-artifact references. Download authorization rechecks current owner/admin membership, derives escaped organization-prefixed keys only from visible rows, rejects client keys, bounds lifetime to 15 minutes, and returns one denial without signer access for cross-organization, viewer, deleted, expired, revoked, or unknown resources. A removed admin cannot refresh an expired link.
- **Verification baseline:** 11 native PostgreSQL integration tests and 78 existing host tests pass with zero skips/failures. JavaScript syntax, JSON, Git whitespace, privacy, role/table policy state, loopback provider behavior, and real macOS parser network denial pass. Hosted run `29403024703` remains the latest Linux release-evidence baseline. No Docker, persistent PostgreSQL service, or real object provider was used.
- **Blocking evidence:** D-002 remains proposed because the full API role matrix, a real queue, real provider-side URL/object/deletion behavior, observable privileged access, remaining resource types, and deletion paths are not yet executable. Broader fixture coverage remains permission-gated; production D-012 gates remain separate.
- **Next technical action:** continue P0-05 with a real-queue proof that requires organization ID plus domain ID in every durable payload, rejects ID-only work before enqueue and execution, and preserves Alpha/Beta isolation across retry.
...
| P0-05 organization isolation | Active; relational and signer boundaries proven | Native Postgres forced RLS, composite ownership, pooled context, repositories, jobs, aggregates, relational exports, derived object keys, bounded downloads, uniform denial, and membership revocation pass; full API roles, real queue/provider, privileged access, remaining resources, and deletion remain open. |
