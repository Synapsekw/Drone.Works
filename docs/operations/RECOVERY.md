...
|---|---|---:|---|---|
| PostgreSQL | RDS automated backups and point-in-time logs | 35 days production; 7 days staging | engineering on-call | restore privately, validate, replay deletion receipts, then expose |
...
3. Verify migration checksums, table ownership, grants, forced RLS, row counts,
   and the isolation-contract digest.
...
6. Start API and worker in restricted mode; run synthetic Alpha/Beta isolation,
   queue, download, and parser smoke tests.
