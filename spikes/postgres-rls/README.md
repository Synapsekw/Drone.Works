...
The schema deliberately stays small: organizations, memberships, pilot profiles, aircraft, canonical flights, immutable flight revisions, and telemetry samples. Every customer-owned child carries `organization_id`; composite foreign keys prevent a resource in one organization from referencing a parent in another.
...
- all seven customer-owned tables enable and force row-level security;
...
The spike does not yet prove API role behavior, an actual job queue, object-storage keys or signed downloads, Drizzle migration generation, privileged maintenance observability, or organization deletion. Those remain P0-05 follow-ups documented in [`../../docs/architecture/TENANCY.md`](../../docs/architecture/TENANCY.md).
