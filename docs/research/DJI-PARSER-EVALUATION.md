...
The CI proof uses generated bytes only. It tests denied network, write, and child-process attempts; crash isolation; a successful operation after a crash; wall-time and output termination; and cgroup OOM classification. Local development does not require a container runtime. Until the first Linux workflow run passes, the hard-container checklist item remains open and this implementation is not acceptance evidence for D-009.
