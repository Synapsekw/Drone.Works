...
This is a conditional feasibility recommendation, not production acceptance of the community parser. Frame correctness, authorized v14 decoding, supply-chain remediation, hard container limits, and the key-service/legal gates remain open.
...
| Local fixture evidence | Initializes all three local v14 candidates and detects version 14 | No documented v14 support and not run because it requires an App Key |
...
If authorized v14 decoding succeeds, compare two production packaging options from the same reviewed source:
